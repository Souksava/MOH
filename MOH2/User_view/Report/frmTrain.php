    ...
    <div class="container font14">
        ...
        <div class="table-responsive">
            <table class="table table-striped" style="width: 2000px;">
                ...
                <?php
                    $sqlsearch = "select t.train_id,sec_name,count(d.per_id) as qty,place,topic,date_start,date_end,datediff(date_end,date_start) as day,year,quota_name,amount,region,No_,t.note from training t left join tranddetail d on t.train_id=d.train_id left join personality p on d.per_id=p.per_id left join section s on p.sec_id=s.sec_id group by p.sec_id;";
                    $resultsearch = mysqli_query($link,$sqlsearch);
                    $No_ = 0;
                    while($row = mysqli_fetch_array($resultsearch, MYSQLI_ASSOC)){
                ?>
